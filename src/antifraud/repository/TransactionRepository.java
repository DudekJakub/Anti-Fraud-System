...
//    @Query(value = "SELECT * FROM transactions WHERE number = :number AND date <= :#{@transactionRepository.oneHourBefore()}", nativeQuery = true)
    ...
    List<Transaction> getOneHourOldTransactionsByNumber(@Param("number") String number, @Param("givenDateMinusHour") LocalDateTime givenDateMinusHour, @Param("givenDate") LocalDateTime givenDate);
